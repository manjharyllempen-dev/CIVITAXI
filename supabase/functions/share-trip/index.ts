...
const valid=(lat:any,lng:any)=>Number.isFinite(Number(lat))&&Number.isFinite(Number(lng))&&Math.abs(Number(lat))<=90&&Math.abs(Number(lng))<=180
...
Deno.serve(async (req:Request)=>{
  ...
  const currentLat=valid(last?.lat,last?.lng)?Number(last.lat):Number(trip.origin_lat)
  const currentLng=valid(last?.lat,last?.lng)?Number(last.lng):Number(trip.origin_lng)
  const hasCurrent=valid(currentLat,currentLng)
  const hasDestination=valid(trip.destination_lat,trip.destination_lng)
  const currentMap=hasCurrent?`https://www.google.com/maps/search/?api=1&query=${encodeURIComponent(currentLat+','+currentLng)}`:''
  const routeMap=hasCurrent&&hasDestination?`https://www.google.com/maps/dir/?api=1&origin=${encodeURIComponent(currentLat+','+currentLng)}&destination=${encodeURIComponent(Number(trip.destination_lat)+','+Number(trip.destination_lng))}&travelmode=driving`:''
  const maps=hasCurrent?`<a class="btn" href="${currentMap}" target="_blank" rel="noopener">📍 ABRIR UBICACIÓN ACTUAL EN GOOGLE MAPS</a>${routeMap?`<a class="btn alt" href="${routeMap}" target="_blank" rel="noopener">🗺️ VER RUTA AL DESTINO EN GOOGLE MAPS</a>`:''}<p class="muted small">La ubicación se actualiza en CiviTaxi cada 12 segundos. Abre Google Maps para ver el punto o la ruta.</p>`:'<p>Ubicación en vivo todavía no disponible.</p>'
  const html=`<!doctype html><html lang="es"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><meta http-equiv="refresh" content="12"><title>CiviTaxi · Viaje compartido</title><style>body{margin:0;font-family:system-ui;background:#E6007E;color:#fff}.wrap{max-width:520px;margin:auto;padding:24px}.card{background:#EF3B99;padding:20px;border-radius:18px;margin:16px 0}.btn{display:block;background:#B50063;color:#fff;text-decoration:none;text-align:center;padding:15px;border-radius:14px;font-weight:800;margin:10px 0}.btn.alt{background:#fff;color:#7d0046}.muted{opacity:.88}.small{font-size:13px;line-height:1.45}h1{margin:0 0 8px}</style></head><body><div class="wrap"><h1>🚕 CiviTaxi</h1><div class="muted">Viaje compartido temporal · se actualiza cada 12 s</div><div class="card"><b>Estado</b><h2>${esc(status)}</h2><p>📍 ${esc(trip.origin_address)}</p><p>🏁 ${esc(trip.destination_address)}</p><p>Tarifa estimada: S/ ${Number(trip.estimated_fare||0).toFixed(2)}</p></div><div class="card"><b>Chofer</b><p>${esc(driver?.full_name||'Buscando chofer')}</p>${vehicle?`<p>${esc(vehicle.brand)} ${esc(vehicle.model)} · ${esc(vehicle.color)} · ${esc(vehicle.plate)}</p>`:''}${maps}</div><p class="muted">Este enlace dejará de funcionar al expirar o ser revocado.</p></div></body></html>`
