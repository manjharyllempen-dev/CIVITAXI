...
const Civi={
 ...
 async ensureVehicle(plate,photoUrl=''){const uid=this.userId();const rows=await this.json(SUPABASE_URL+'/rest/v1/vehicles?driver_id=eq.'+encodeURIComponent(uid)+'&select=*&order=created_at.desc&limit=1',{headers:this.headers()});const body={plate:String(plate||'').trim().toUpperCase(),active:true};if(photoUrl)body.photo_url=photoUrl;if(rows[0])return this.json(SUPABASE_URL+'/rest/v1/vehicles?id=eq.'+encodeURIComponent(rows[0].id),{method:'PATCH',headers:{...this.headers(),'Prefer':'return=representation'},body:JSON.stringify(body)});return this.json(SUPABASE_URL+'/rest/v1/vehicles',{method:'POST',headers:{...this.headers(),'Prefer':'return=representation'},body:JSON.stringify({...body,driver_id:uid})})},
