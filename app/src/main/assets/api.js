...
const Civi={
 ...
 async json(url,opts={},retry=true){let r=await fetch(url,opts);if(r.status===401&&retry&&this.refreshToken()&&!url.includes('/auth/v1/token')){await this.refreshSession();const h={...(opts.headers||{}),Authorization:'Bearer '+this.token(),apikey:SUPABASE_KEY};return this.json(url,{...opts,headers:h},false)}let j=null;try{j=await r.json()}catch(e){j=null}if(!r.ok)throw new Error((j&&(j.msg||j.message||j.error_description||j.hint))||('HTTP '+r.status));return j},
 async signUp(email,password,fullName,role='usuario'){const j=await this.json(SUPABASE_URL+'/auth/v1/signup',{method:'POST',headers:this.headers(false),body:JSON.stringify({email,password,data:{full_name:fullName,role}})});return this.saveSession(j)},
