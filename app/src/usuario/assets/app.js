...
function installPasswordVisibility(){document.querySelectorAll('input[type="password"]').forEach(input=>{if(input.dataset.civiPassword)return;input.dataset.civiPassword='1';const wrap=document.createElement('div');wrap.className='password-wrap';input.parentNode.insertBefore(wrap,input);wrap.appendChild(input);const b=document.createElement('button');b.type='button';b.className='password-toggle';b.textContent='👁';b.setAttribute('aria-label','Mostrar contraseña');b.setAttribute('aria-pressed','false');wrap.appendChild(b);b.onclick=()=>{const hidden=input.type==='password';input.type=hidden?'text':'password';b.textContent=hidden?'🙈':'👁';b.setAttribute('aria-label',hidden?'Ocultar contraseña':'Mostrar contraseña');b.setAttribute('aria-pressed',String(hidden));input.focus()}})}
function installPassengerPhotoRegistration(){const section=document.getElementById('register');if(!section||document.getElementById('userPhoto'))return;const registerButton=[...section.querySelectorAll('button')].find(b=>/REGISTRARME/i.test(b.textContent||''));if(!registerButton)return;const label=document.createElement('label');label.style.cssText='display:block;padding:12px;border:1px dashed #ffc72c88;border-radius:14px;margin:9px 0;background:#ffffff08';label.innerHTML='📷 Foto de perfil del usuario<input id="userPhoto" type="file" accept="image/*" style="display:block;margin-top:7px;width:100%">';section.insertBefore(label,registerButton);const original=window.userRegister;if(typeof original==='function'){window.userRegister=async function(){const before=Civi.userId(),file=document.getElementById('userPhoto')?.files?.[0]||null;await original();const after=Civi.userId();if(file&&after&&after!==before){try{const u=await Civi.uploadFile('civitaxi-public',file,'perfil');await Civi.updateProfile({avatar_url:u.url})}catch(e){console.warn('Foto de usuario pendiente',e)}}}}}

const CIVI_MANUAL_ROUTE_KEY='civitaxi_usuario_direccion_manual_v1';
function civiPassenger(){return true}
