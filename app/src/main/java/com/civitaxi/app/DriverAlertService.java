...
public class DriverAlertService extends Service {
  ...
  private void checkRequests(){try{String token=prefs().getString("access_token","");if(token.isEmpty())return;HttpURLConnection c=open("/rest/v1/trips?select=id&status=eq.solicitado&driver_id=is.null&order=requested_at.desc&limit=1",token,"GET");int code=c.getResponseCode();if(code==401&&refreshToken()){token=prefs().getString("access_token","");c=open("/rest/v1/trips?select=id&status=eq.solicitado&driver_id=is.null&order=requested_at.desc&limit=1",token,"GET");code=c.getResponseCode();}if(code<200||code>=300)return;JSONArray rows=new JSONArray(read(c));if(rows.length()==0){lastTripId="";return;}String id=rows.getJSONObject(0).optString("id","");if(!id.isEmpty()&&!id.equals(lastTripId)){lastTripId=id;showRequestAlert();}}catch(Exception ignored){}}
